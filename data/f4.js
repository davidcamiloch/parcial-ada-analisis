...
(function () {
  ...
  reg({
    ...
    def: () => ({
      ...
      Tn: { prof: '6 + 12floor(n/2)', hojas: '5 + 10floor(n/2)' }, bigO: 'n', omega: 'n', sinCasos: true,
      mejorPeor: 'No hay mejor ni peor caso: siempre ⌊n/2⌋ intercambios. O(n) = Ω(n).',
      pasos: [{ txt: '⌊n/2⌋ vueltas de costo 2 + 3 + 3 + 2 + 2 = 12:', tex: 'T(n) = 2 + 2 + 12\\lfloor n/2 \\rfloor + 2 = 6 + 12\\lfloor n/2\\rfloor \\approx 6n + 6 \\in O(n)' }],
  ...
  reg({
    ...
    def: () => ({
        ...
        FOR('fi', 'for (int i = 0; i < n; i++) {', 'int i = 0', 'i < n', 'i++', { tipo: 'for', var: 'i', desde: '0', hasta: 'n-1', t: 'n', total: 'n', invariante: 'i == n' },
          [{ id: 'fj', txt: 'for (int j = 0; j < n; j++) {', cierre: '}', ciclo: { tipo: 'for', var: 'j', desde: '0', hasta: 'n-1', t: 'n', total: 'n^2', valores: 'peor caso (sin comunes): n vueltas por cada i; mejor caso: 1 (break)' },
            ...
            cuerpo: [{ id: 'if', txt: 'if (A[i] == B[j]) {', cierre: '}', costo: { prof: '3', hojas: '3' }, veces: 'n^2', porque: '2 accesos + comparación = 3.', tag: 'cond', vecesMejor: 'n',
              cuerpo: [L('c1', 'C[k] = A[i];', 2, 2, '0', { porque: 'Acceso + asignación: 2. Peor caso (sin comunes): nunca.', tag: 'rama', vecesMejor: 'n' }), L('c2', 'k++;', 2, 1, '0', { porque: 'Suma + asignación: 2.', tag: 'upd', vecesMejor: 'n' }), L('br', 'break;', 1, 1, '0', { porque: 'break: 1 (salta al siguiente i).', tag: 'ret', vecesMejor: 'n' })] }] }],
          { cond: 'n+1', upd: 'n' }),
        L('ret', 'return C;', 1, 1, 1, { porque: 'return: 1.', tag: 'ret' })
      ],
      datos: { peor: (n) => ({ A: asc(n), B: asc(n).map((x) => x + 1000) }), mejor: (n) => ({ A: Array(n).fill(7), B: Array(n).fill(7) }) }, casoTn: 'peor',
      Tn: { prof: '8 + k + 6n + 6n^2', hojas: '6 + k + 4n + 5n^2' }, TnMejor: { prof: '8 + k + 14n', hojas: '6 + k + 11n' }, bigO: 'n^2', omega: 'n',
      ...
      pasos: [{ txt: 'Peor caso: n² vueltas internas de costo 1 + 3 + 2 = 6; por cada i: 1 + 2 + 6n + 1 + 2:', tex: 'T_{peor}(n) = 8 + k + 6n + 6n^2 \\in O(n^2)' }, { txt: 'Mejor caso: interno 1 vuelta (cond 1 + if 3 + copia 2 + k++ 2 + break 1) por cada i:', tex: 'T_{mejor}(n) = 8 + k + 14n \\in \\Omega(n)' }],
